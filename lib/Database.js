...
class Database {
    ...
    insertUpdateGuildSetting(guildId, name, value) {
        this.knex(constants.TABLE_GUILD_SETTINGS)
            ...
            .then((row) => {
                ...
                if(!row) {
                    attributes.created_at = this.knex.fn.now();
                    this.knex(constants.TABLE_GUILD_SETTINGS).insert(attributes).then((result) => {
    ...
    getGuildSetting(guildId, name) {
        return this.knex(constants.TABLE_GUILD_SETTINGS)
            .where('guild_id', guildId)
            .where('name', name)
            .first('value');
    }
